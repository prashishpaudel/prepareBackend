...
module.exports = {
	...
	getStreamData: function(inputparams, callback) { 
		try {
					query = "SELECT feature, avg(value) as value FROM prepare.physlogicaltable  where creationTimestamp > ?  and SERIALNUMBER = ? and DEVICECONNECTTIME= ? group by feature"
	...
	getLearnersByScenarioid: function(s_id, callback) { 
		try {
			...
			query = "select * from learners where COURSE_ID in(select distinct(course_id) as course_id FROM prepare.SCENARIO where scenario_id=?);"
	...
	getScenarioById: function(s_id, callback) {
		try {
					...
					query = "SELECT * FROM prepare.scenario where scenario_id=? "
	...
	getCourses: function(callback) {
		try {
					query = "SELECT s.*, u.username  FROM prepare.course s left join users u on u.id=s.id and course_id > 35";
	...
	getscenariobycourseid: function(inputdata, callback) {
		try {
						...
						query = "SELECT CATEGORY as DISCIPLINE, SCENARIO_NAME as `SCENARIO NAME`, (TIME_DURATION/60) as 'TIME DURATION', scenario_id, course_id  FROM prepare.scenario where course_id = ? "
