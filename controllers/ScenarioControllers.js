...
module.exports = {
	...
	downloadPhysioData: function (inputdata, callback) {
		try {
			...
			return ScenarioModel.speceficResults(inputdata, function (err1, events) {
				...
				inputdata.serialNumber = device_id;
				inputdata.startTime = start - 60000;
				//inputdata.endTime=end+45000;
				inputdata.endTime = end + 60000;
	...
	deleteLearner: function (inputdata, callback) {
		try {
			return ScenarioModel.deleteLearner(inputdata, function (err, results) {
				if (err) {
					console.log(err)
					throw err;
				}
	...
	getAudioStream: function (callback) {
		try {
			return ScenarioModel.getAudioStream(function (err, results) {
				if (err) {
					throw err;
				}
				...
				var data = {
					error: 0,
					data: results,
					message: 'Retrieved Audio Stream Data'
				}
	...
	addAudioStream: function (inputdata, callback) {
		try {
			...
			return ScenarioModel.addAudioStream(inputdata, function (err1, results) {
				if (err1) {
					console.log(err1)
					throw err1;
				}
	...
	deleteAudioStream: function (id, callback) {
		try {
			return ScenarioModel.deleteAudioStream(id, function (err, results) {
				if (err) {
					throw err;
				}
				...
				var message = {
					error: 0,
					message: 'Deleted Audio Stream Data Successfully'
				}
